demo = {
  ...
  showSwal: function(type, title, txt, func) {
    if (type == 'basic') {
      swal({
      ...
      swal({
        ...
        if (obj.value == true)
          timelineController.deletePost(func);
      })
      ...
      swal({
        ...
        swal({
          ...
          console.log(obj);
          if (obj.value == true)
            contentController.register(false, title);
        }).catch(swal.noop);
        let pane = document.querySelector("#pane");
        loadView.getView(pane, 'contentRegister');
        //document.querySelector("#local-preview").style = "max-width: 100%;";
       
        
        contentController.setEditPost(title);
